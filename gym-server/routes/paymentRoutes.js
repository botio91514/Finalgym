...
// Create payment for store sale
router.post('/create-store', async (req, res) => {
  try {
    const { saleId, amount, customerName } = req.body;
    if (!saleId || !amount) {
      return res.status(400).json({ status: 'error', message: 'saleId and amount required' });
    }
    const { payment } = await createStorePayment({ saleId, amount, customerName });
    return res.status(201).json({
      status: 'success',
      data: {
        orderId: payment.orderId,
