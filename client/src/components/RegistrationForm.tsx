...
import { getPlanPricing, getActiveOffers } from '../utils/apiClient';
...
const RegistrationForm: React.FC = () => {
  ...
  // Fetch plan pricing and offers from settings on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Update plans with fetched pricing and apply offers
        setPlans(prevPlans =>
          prevPlans.map(plan => {
            const originalPrice = pricing[plan.id] || plan.price;
            let finalPrice = originalPrice;
            let appliedOffer = undefined;

            // Find applicable offer for this plan
            if (offers && offers.length > 0) {
              const applicableOffer = offers.find(offer =>
                offer.isActive &&
                offer.applicablePlans.includes(plan.id)
              );

              if (applicableOffer) {
                if (applicableOffer.discountType === 'percentage') {
                  const discountAmount = (originalPrice * applicableOffer.discountValue) / 100;
                  finalPrice = originalPrice - discountAmount;
                } else {
                  finalPrice = originalPrice - applicableOffer.discountValue;
                }
            ...
            return {
              ...plan,
              price: finalPrice,
              originalPrice: appliedOffer ? originalPrice : undefined,
              offer: appliedOffer
            };
          })
        );
    ...
    <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {plans.map((plan) => (
              <button
                ...
                className={`p-4 rounded-xl transition-all duration-300 transform hover:scale-105 focus:ring-2 focus:ring-yellow-300 focus:outline-none relative ${selectedPlan === plan.id
              ...
              >
                {plan.offer && (
                  <div className="absolute top-0 right-0 transform translate-x-2 -translate-y-2">
                    <span className={`px-2 py-1 text-xs font-bold rounded-full ${selectedPlan === plan.id ? 'bg-white text-yellow-600' : 'bg-yellow-500 text-white'
                      }`}>
                ...
                <div className="flex flex-col items-center">
                  {plan.originalPrice && plan.price < plan.originalPrice && (
                    <span className={`text-sm line-through ${selectedPlan === plan.id ? 'text-gray-700' : 'text-gray-400'
                      }`}>
                      {formatPrice(plan.originalPrice)}
                    </span>
                  )}
                  <div className={`text-2xl font-bold ${selectedPlan === plan.id ? 'text-black' : 'text-yellow-500'
                    }`}>
                ...
                {plan.offer && (
                  <div className={`mt-2 text-xs ${selectedPlan === plan.id ? 'text-gray-800' : 'text-gray-500'
                    }`}>
                    {plan.offer.name}
                  </div>
                )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                type="text"
                className={`mt-1 block w-full px-4 py-3 rounded-lg border-2 shadow-sm focus:ring-2 focus:ring-yellow-300 transition-all duration-200 ${errors.name ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-yellow-500'
                  }`}
                value={formData.name}
                onChange={(e) => {
            ...
            <div className="space-y-2">
              ...
              <input
                type="email"
                className={`mt-1 block w-full px-4 py-3 rounded-lg border-2 shadow-sm focus:ring-2 focus:ring-yellow-300 transition-all duration-200 ${errors.email ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-yellow-500'
                  }`}
            ...
            <div className="space-y-2">
              ...
              <input
                type="tel"
                className={`mt-1 block w-full px-4 py-3 rounded-lg border-2 shadow-sm focus:ring-2 focus:ring-yellow-300 transition-all duration-200 ${errors.phone ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-yellow-500'
                  }`}
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <textarea
                className={`mt-1 block w-full px-4 py-3 rounded-lg border-2 shadow-sm focus:ring-2 focus:ring-yellow-300 transition-all duration-200 resize-none ${errors.address ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-yellow-500'
                  }`}
                rows={3}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                <div className={`w-32 h-32 border-2 ${errors.photo ? 'border-red-500' : 'border-gray-200'
                  } border-dashed rounded-xl flex items-center justify-center transition-all duration-200 hover:border-yellow-500 ${errors.photo ? 'bg-red-50' : ''
                  }`}>
                  <img
                    src={photoPreview}
                    alt="Profile"
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={isLoading}
              className={`px-6 py-3 text-lg font-semibold rounded-lg shadow-lg hover:shadow-xl transform transition-all duration-200 ${isLoading
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-sm w-full shadow-2xl flex flex-col items-center animate-slideIn">
            <div className="w-16 h-16 mb-4 relative">
              <div className="absolute inset-0 border-4 border-gray-200 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-yellow-500 rounded-full border-t-transparent animate-spin"></div>
            </div>
            <h3 className="text-xl font-bold text-gray-800 mb-2">Processing Registration</h3>
            <p className="text-gray-600 text-center">Please wait while we create your account...</p>
          </div>
        </div>
      )}
    </div>
