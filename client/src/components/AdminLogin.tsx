...
const AdminLogin: React.FC<AdminLoginProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    // Clear session storage when the login page loads
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.resetToken) {
        ...
        setTimeout(() => {
          navigate(`/admin/reset-password/${data.resetToken}`);
        }, 1000);
      }
    } catch (err) {
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className={`w-full px-6 py-3 text-lg font-semibold text-white rounded-lg shadow-lg hover:shadow-xl transform transition-all duration-200 ${isLoading
              ...
              {showOTPVerification ? (
                <form onSubmit={handleVerifyOTP} className="space-y-6">
                  ...
                  <button
                    ...
                    className={`w-full px-6 py-3 text-lg font-semibold text-white rounded-lg shadow-lg hover:shadow-xl transform transition-all duration-200 ${otpLoading || otp.length !== 6
                      ? 'opacity-50 cursor-not-allowed bg-yellow-500'
                      : 'bg-yellow-500 hover:bg-yellow-600 hover:-translate-y-0.5'
                      }`}
                  >
                ...
                <form onSubmit={handleForgotPassword} className="space-y-6">
                  ...
                  <button
                    type="submit"
                    disabled={forgotPasswordLoading}
                    className={`w-full px-6 py-3 text-lg font-semibold text-white rounded-lg shadow-lg hover:shadow-xl transform transition-all duration-200 ${forgotPasswordLoading
                      ? 'opacity-50 cursor-not-allowed bg-yellow-500'
                      : 'bg-yellow-500 hover:bg-yellow-600 hover:-translate-y-0.5'
                      }`}
                  >
